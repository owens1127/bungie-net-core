 ...
 */
export async function getTrendingCategories(http: BungieHttpProtocol): Promise<BungieNetResponse<TrendingCategories>> {
  const baseUrl = `https://www.bungie.net/Platform/Trending/Categories/`;
  const searchParams = undefined;
  return await http<BungieNetResponse<TrendingCategories>>({ method: 'GET', baseUrl, searchParams, body: undefined });
}
...
export async function getTrendingCategory(
  ...
  const searchParams = undefined;
  return await http<BungieNetResponse<SearchResultOfTrendingEntry>>({
    method: 'GET',
    baseUrl,
    searchParams,
    body: undefined
...
export async function getTrendingEntryDetail(
  ...
  const searchParams = undefined;
  return await http<BungieNetResponse<TrendingDetail>>({ method: 'GET', baseUrl, searchParams, body: undefined });
}
