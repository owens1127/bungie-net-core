...
class QueueItem {
  ...
  async execute(retry) {
    ...
    return fetch(this.url, this.init).then(response => response.json()).then(res => {
...
class ManifestQueueItem extends QueueItem {
  constructor(url, init, resolve, reject) {
    super(url, init, resolve, reject);
  }
  async execute(retry) {
    return fetch(this.url, this.init).then(response => response.json()
    .then(res => this.resolve(res)).catch(e => this.reject(e))).then(() => 0);
  }
}
