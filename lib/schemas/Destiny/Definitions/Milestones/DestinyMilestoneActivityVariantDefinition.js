 ...
 * @property {number} activityHash The hash to use for looking up the variant Activity's definition (DestinyActivityDefinition), where you can find its distinguishing characteristics such as difficulty level and recommended light level. 
Frequently, that will be the only distinguishing characteristics in practice, which is somewhat of a bummer. Mapped to DestinyActivityDefinition in the manifest.
 * @property {number} order If you care to do so, render the variants in the order prescribed by this value.
...
module.exports = DestinyMilestoneActivityVariantDefinition;
