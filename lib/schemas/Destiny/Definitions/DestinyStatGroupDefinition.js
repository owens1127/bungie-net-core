 ...
 * @property {number} maximumValue The maximum possible value that any stat in this group can be transformed into.
 ...
 * @property {number} uiPosition This apparently indicates the position of the stats in the UI? I've returned it in case anyone can use it, but it's not of any use to us on BNet. Something's being lost in translation with this value.
 * @property {DestinyStatDisplayDefinition[]} scaledStats Any stat that requires scaling to be transformed from an "Investment" stat to a "Display" stat will have an entry in this list. For more information on what those types of stats mean and the transformation process, see DestinyStatDefinition.
 ...
 * @property {Object.key<number, DestinyStatOverrideDefinition>} overrides The game has the ability to override, based on the stat group, what the localized text is that is displayed for Stats being shown on the item.
...
module.exports = DestinyStatGroupDefinition;
