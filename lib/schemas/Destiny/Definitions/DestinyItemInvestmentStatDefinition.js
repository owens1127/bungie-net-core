 ...
 * @property {number} statTypeHash The hash identifier for the DestinyStatDefinition defining this stat. Mapped to DestinyStatDefinition in the manifest.
 * @property {number} value The raw "Investment" value for the stat, before transformations are performed to turn this raw stat into stats that are displayed in the game UI.
 * @property {boolean} isConditionallyActive If this is true, the stat will only be applied on the item in certain game state conditions, and we can't know statically whether or not this stat will be applied. Check the "live" API data instead for whether this value is being applied on a specific instance of the item in question, and you can use this to decide whether you want to show the stat on the generic view of the item, or whether you want to show some kind of caveat or warning about the stat value being conditional on game state.
...
module.exports = DestinyItemInvestmentStatDefinition;
