 ...
 * @property {DestinyProgressionDisplayPropertiesDefinition} displayProperties 
 * @property {DestinyProgressionScope} scope The "Scope" of the progression indicates the source of the progression's live data.
 ...
 * @property {boolean} repeatLastStep If this is True, then the progression doesn't have a maximum level.
 * @property {string} source If there's a description of how to earn this progression in the local config, this will be that localized description.
 * @property {DestinyProgressionStepDefinition[]} steps Progressions are divided into Steps, which roughly equate to "Levels" in the traditional sense of a Progression. Notably, the last step can be repeated indefinitely if repeatLastStep is true, meaning that the calculation for your level is not as simple as comparing your current progress to the max progress of the steps. 
 ...
 * @property {boolean} visible If true, the Progression is something worth showing to users.
If false, BNet isn't going to show it. But that doesn't mean you can't. We're all friends here.
 * @property {number?} factionHash If the value exists, this is the hash identifier for the Faction that owns this Progression.
This is purely for convenience, if you're looking at a progression and want to know if and who it's related to in terms of Faction Reputation. Mapped to DestinyFactionDefinition in the manifest.
 * @property {DestinyColor} color The #RGB string value for the color related to this progression, if there is one.
 * @property {string} rankIcon For progressions that have it, this is the rank icon we use in the Companion, displayed above the progressions' rank value.
 * @property {DestinyProgressionRewardItemQuantity[]} rewardItems 
...
module.exports = DestinyProgressionDefinition;
