 ...
 * @property {BucketScope} scope Where the bucket is found. 0 = Character, 1 = Account
 * @property {BucketCategory} category An enum value for what items can be found in the bucket. See the BucketCategory enum for more details.
 * @property {number} bucketOrder Use this property to provide a quick-and-dirty recommended ordering for buckets in the UI. Most UIs will likely want to forsake this for something more custom and manual.
 * @property {number} itemCount The maximum # of item "slots" in a bucket. A slot is a given combination of item + quantity.
 ...
 * @property {ItemLocation} location Sometimes, inventory buckets represent conceptual "locations" in the game that might not be expected. This value indicates the conceptual location of the bucket, regardless of where it is actually contained on the character/account. 
 ...
 * @property {boolean} hasTransferDestination If TRUE, there is at least one Vendor that can transfer items to/from this bucket. See the DestinyVendorDefinition's acceptedItems property for more information on how transferring works.
 * @property {boolean} enabled If True, this bucket is enabled. Disabled buckets may include buckets that were included for test purposes, or that were going to be used but then were abandoned but never removed from content *cough*.
 * @property {boolean} fifo if a FIFO bucket fills up, it will delete the oldest item from said bucket when a new item tries to be added to it. If this is FALSE, the bucket will not allow new items to be placed in it until room is made by the user manually deleting items from it. You can see an example of this with the Postmaster's bucket.
...
module.exports = DestinyInventoryBucketDefinition;
