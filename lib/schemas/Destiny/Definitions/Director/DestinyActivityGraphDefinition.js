 ...
 * @property {DestinyActivityGraphNodeDefinition[]} nodes These represent the visual "nodes" on the map's view. These are the activities you can click on in the map.
 * @property {DestinyActivityGraphArtElementDefinition[]} artElements Represents one-off/special UI elements that appear on the map.
 * @property {DestinyActivityGraphConnectionDefinition[]} connections Represents connections between graph nodes. However, it lacks context that we'd need to make good use of it.
 * @property {DestinyActivityGraphDisplayObjectiveDefinition[]} displayObjectives Objectives can display on maps, and this is supposedly metadata for that. I have not had the time to analyze the details of what is useful within however: we could be missing important data to make this work. Expect this property to be expanded on later if possible.
 * @property {DestinyActivityGraphDisplayProgressionDefinition[]} displayProgressions Progressions can also display on maps, but similarly to displayObjectives we appear to lack some required information and context right now. We will have to look into it later and add more data if possible.
 * @property {DestinyLinkedGraphDefinition[]} linkedGraphs Represents links between this Activity Graph and other ones.
...
module.exports = DestinyActivityGraphDefinition;
