 ...
 * @property {number} talentGridHash Most items don't have useful talent grids anymore, but Builds in particular still do.
You can use this hash to lookup the DestinyTalentGridDefinition attached to this item, which will be crucial for understanding the node values on the item. Mapped to DestinyTalentGridDefinition in the manifest.
 * @property {DestinyTalentNode[]} nodes Detailed information about the individual nodes in the talent grid.
 ...
 * @property {boolean} isGridComplete Indicates whether the talent grid on this item is completed, and thus whether it should have a gold border around it.
 ...
 * @property {DestinyProgression} gridProgression If the item has a progression, it will be detailed here. A progression means that the item can gain experience. Thresholds of experience are what determines whether and when a talent node can be activated.
...
module.exports = DestinyItemTalentGridComponent;
