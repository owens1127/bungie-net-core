...
import { getPublicVendorsTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getPublicVendors } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getPublicVendors', () => { 
  test('the endpoint exists', () => { 
    expect(getPublicVendors).toBeDefined();
  })

  getPublicVendorsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getPublicVendors> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getPublicVendors(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getPublicVendors>>))
        }
    })
  ));
})
