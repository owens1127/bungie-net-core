...
import { transferItemTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { transferItem } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.transferItem', () => { 
  test('the endpoint exists', () => { 
    expect(transferItem).toBeDefined();
  })

  transferItemTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof transferItem> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await transferItem(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof transferItem>>))
        }
    })
  ));
})
