...
import { reportOffensivePostGameCarnageReportPlayer } from '../../../src/endpoints/Destiny2';
...
describe('Destiny2.reportOffensivePostGameCarnageReportPlayer', () => { 
  test('the endpoint exists', () => { 
    expect(reportOffensivePostGameCarnageReportPlayer).toBeDefined();
  })

  reportOffensivePostGameCarnageReportPlayerTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof reportOffensivePostGameCarnageReportPlayer> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await reportOffensivePostGameCarnageReportPlayer(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof reportOffensivePostGameCarnageReportPlayer>>))
        }
    })
  ));
})
