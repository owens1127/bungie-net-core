...
import { searchDestinyPlayerByBungieName } from '../../../src/endpoints/Destiny2';
...
describe('Destiny2.searchDestinyPlayerByBungieName', () => { 
  test('the endpoint exists', () => { 
    expect(searchDestinyPlayerByBungieName).toBeDefined();
  })

  searchDestinyPlayerByBungieNameTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof searchDestinyPlayerByBungieName> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await searchDestinyPlayerByBungieName(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof searchDestinyPlayerByBungieName>>))
        }
    })
  ));
})
