...
import { getDestinyManifestTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getDestinyManifest } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getDestinyManifest', () => { 
  test('the endpoint exists', () => { 
    expect(getDestinyManifest).toBeDefined();
  })

  getDestinyManifestTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getDestinyManifest> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getDestinyManifest(sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getDestinyManifest>>))
        }
    })
  ));
})
