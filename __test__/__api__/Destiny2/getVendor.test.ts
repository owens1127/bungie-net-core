...
import { getVendorTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getVendor } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getVendor', () => { 
  test('the endpoint exists', () => { 
    expect(getVendor).toBeDefined();
  })

  getVendorTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getVendor> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getVendor(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getVendor>>))
        }
    })
  ));
})
