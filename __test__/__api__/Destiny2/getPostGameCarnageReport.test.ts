...
import { getPostGameCarnageReport } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getPostGameCarnageReport', () => { 
  test('the endpoint exists', () => { 
    expect(getPostGameCarnageReport).toBeDefined();
  })

  getPostGameCarnageReportTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getPostGameCarnageReport> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getPostGameCarnageReport(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getPostGameCarnageReport>>))
        }
    })
  ));
})
