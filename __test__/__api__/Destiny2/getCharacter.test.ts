...
import { getCharacterTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getCharacter } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getCharacter', () => { 
  test('the endpoint exists', () => { 
    expect(getCharacter).toBeDefined();
  })

  getCharacterTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getCharacter> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getCharacter(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getCharacter>>))
        }
    })
  ));
})
