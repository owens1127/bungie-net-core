...
import { insertSocketPlugTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { insertSocketPlug } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.insertSocketPlug', () => { 
  test('the endpoint exists', () => { 
    expect(insertSocketPlug).toBeDefined();
  })

  insertSocketPlugTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof insertSocketPlug> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await insertSocketPlug(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof insertSocketPlug>>))
        }
    })
  ));
})
