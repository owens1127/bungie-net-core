...
import { equipItemTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { equipItem } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.equipItem', () => { 
  test('the endpoint exists', () => { 
    expect(equipItem).toBeDefined();
  })

  equipItemTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof equipItem> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await equipItem(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof equipItem>>))
        }
    })
  ));
})
