...
import { clearLoadoutTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { clearLoadout } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.clearLoadout', () => { 
  test('the endpoint exists', () => { 
    expect(clearLoadout).toBeDefined();
  })

  clearLoadoutTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof clearLoadout> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await clearLoadout(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof clearLoadout>>))
        }
    })
  ));
})
