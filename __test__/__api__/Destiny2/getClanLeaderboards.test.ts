...
import { getClanLeaderboards } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getClanLeaderboards', () => { 
  test('the endpoint exists', () => { 
    expect(getClanLeaderboards).toBeDefined();
  })

  getClanLeaderboardsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getClanLeaderboards> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getClanLeaderboards(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getClanLeaderboards>>))
        }
    })
  ));
})
