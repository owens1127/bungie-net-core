...
import { getProfileTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getProfile } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getProfile', () => { 
  test('the endpoint exists', () => { 
    expect(getProfile).toBeDefined();
  })

  getProfileTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getProfile> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getProfile(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getProfile>>))
        }
    })
  ));
})
