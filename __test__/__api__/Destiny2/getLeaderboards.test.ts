...
import { getLeaderboardsTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getLeaderboards } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getLeaderboards', () => { 
  test('the endpoint exists', () => { 
    expect(getLeaderboards).toBeDefined();
  })

  getLeaderboardsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getLeaderboards> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getLeaderboards(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getLeaderboards>>))
        }
    })
  ));
})
