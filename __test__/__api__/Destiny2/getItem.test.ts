...
import { getItemTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getItem } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getItem', () => { 
  test('the endpoint exists', () => { 
    expect(getItem).toBeDefined();
  })

  getItemTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getItem> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getItem(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getItem>>))
        }
    })
  ));
})
