...
import { equipItemsTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { equipItems } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.equipItems', () => { 
  test('the endpoint exists', () => { 
    expect(equipItems).toBeDefined();
  })

  equipItemsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof equipItems> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await equipItems(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof equipItems>>))
        }
    })
  ));
})
