...
import { getVendorsTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { getVendors } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.getVendors', () => { 
  test('the endpoint exists', () => { 
    expect(getVendors).toBeDefined();
  })

  getVendorsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getVendors> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getVendors(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getVendors>>))
        }
    })
  ));
})
