...
import { setItemLockStateTests } from '../../Destiny2'
import { describe, test, it, expect } from '@jest/globals';
import { setItemLockState } from '../../../src/endpoints/Destiny2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Destiny2.setItemLockState', () => { 
  test('the endpoint exists', () => { 
    expect(setItemLockState).toBeDefined();
  })

  setItemLockStateTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof setItemLockState> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await setItemLockState(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof setItemLockState>>))
        }
    })
  ));
})
