...
import { getContentType } from '../../../src/endpoints/Content';
...
describe('Content.getContentType', () => { 
  ...
  getContentTypeTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getContentType> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getContentType(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getContentType>>))
        }
    })
  ));
})
