...
import { rssNewsArticles } from '../../../src/endpoints/Content';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Content.rssNewsArticles', () => { 
  test('the endpoint exists', () => { 
    expect(rssNewsArticles).toBeDefined();
  })

  rssNewsArticlesTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof rssNewsArticles> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await rssNewsArticles(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof rssNewsArticles>>))
        }
    })
  ));
})
