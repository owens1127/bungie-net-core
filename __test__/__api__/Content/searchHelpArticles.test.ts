...
import { searchHelpArticles } from '../../../src/endpoints/Content';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Content.searchHelpArticles', () => { 
  test('the endpoint exists', () => { 
    expect(searchHelpArticles).toBeDefined();
  })

  searchHelpArticlesTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof searchHelpArticles> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await searchHelpArticles(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof searchHelpArticles>>))
        }
    })
  ));
})
