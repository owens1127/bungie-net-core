...
import { getContentByIdTests } from '../../Content'
import { describe, test, it, expect } from '@jest/globals';
import { getContentById } from '../../../src/endpoints/Content';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Content.getContentById', () => { 
  test('the endpoint exists', () => { 
    expect(getContentById).toBeDefined();
  })

  getContentByIdTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getContentById> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getContentById(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getContentById>>))
        }
    })
  ));
})
