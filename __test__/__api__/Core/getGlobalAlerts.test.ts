...
import { getGlobalAlertsTests } from '../../Core'
import { describe, test, it, expect } from '@jest/globals';
import { getGlobalAlerts } from '../../../src/endpoints/Core';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Core.getGlobalAlerts', () => { 
  test('the endpoint exists', () => { 
    expect(getGlobalAlerts).toBeDefined();
  })

  getGlobalAlertsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getGlobalAlerts> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getGlobalAlerts(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getGlobalAlerts>>))
        }
    })
  ));
})
