...
import { getCommonSettingsTests } from '../../Core'
import { describe, test, it, expect } from '@jest/globals';
import { getCommonSettings } from '../../../src/endpoints/Core';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Core.getCommonSettings', () => { 
  test('the endpoint exists', () => { 
    expect(getCommonSettings).toBeDefined();
  })

  getCommonSettingsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getCommonSettings> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getCommonSettings(sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getCommonSettings>>))
        }
    })
  ));
})
