...
describe('CommunityContent.getCommunityContent', () => { 
  test('the endpoint exists', () => { 
    expect(getCommunityContent).toBeDefined();
  })

  getCommunityContentTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getCommunityContent> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getCommunityContent(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getCommunityContent>>))
        }
    })
  ));
})
