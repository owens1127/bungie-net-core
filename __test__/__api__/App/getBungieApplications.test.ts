...
import { getBungieApplications } from '../../../src/endpoints/App';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('App.getBungieApplications', () => { 
  test('the endpoint exists', () => { 
    expect(getBungieApplications).toBeDefined();
  })

  getBungieApplicationsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getBungieApplications> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getBungieApplications(sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getBungieApplications>>))
        }
    })
  ));
})
