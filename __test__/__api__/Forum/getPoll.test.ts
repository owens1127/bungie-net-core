...
import { getPollTests } from '../../Forum'
import { describe, test, it, expect } from '@jest/globals';
import { getPoll } from '../../../src/endpoints/Forum';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Forum.getPoll', () => { 
  test('the endpoint exists', () => { 
    expect(getPoll).toBeDefined();
  })

  getPollTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getPoll> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getPoll(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getPoll>>))
        }
    })
  ));
})
