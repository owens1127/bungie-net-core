...
import { getRecruitmentThreadSummaries } from '../../../src/endpoints/Forum';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Forum.getRecruitmentThreadSummaries', () => { 
  test('the endpoint exists', () => { 
    expect(getRecruitmentThreadSummaries).toBeDefined();
  })

  getRecruitmentThreadSummariesTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getRecruitmentThreadSummaries> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getRecruitmentThreadSummaries(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getRecruitmentThreadSummaries>>))
        }
    })
  ));
})
