...
import { getTopicsPagedTests } from '../../Forum'
import { describe, test, it, expect } from '@jest/globals';
import { getTopicsPaged } from '../../../src/endpoints/Forum';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Forum.getTopicsPaged', () => { 
  test('the endpoint exists', () => { 
    expect(getTopicsPaged).toBeDefined();
  })

  getTopicsPagedTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getTopicsPaged> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getTopicsPaged(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getTopicsPaged>>))
        }
    })
  ));
})
