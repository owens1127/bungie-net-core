...
import { getPostAndParentTests } from '../../Forum'
import { describe, test, it, expect } from '@jest/globals';
import { getPostAndParent } from '../../../src/endpoints/Forum';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Forum.getPostAndParent', () => { 
  test('the endpoint exists', () => { 
    expect(getPostAndParent).toBeDefined();
  })

  getPostAndParentTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getPostAndParent> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getPostAndParent(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getPostAndParent>>))
        }
    })
  ));
})
