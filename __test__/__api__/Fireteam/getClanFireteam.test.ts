...
import { getClanFireteamTests } from '../../Fireteam'
import { describe, test, it, expect } from '@jest/globals';
import { getClanFireteam } from '../../../src/endpoints/Fireteam';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Fireteam.getClanFireteam', () => { 
  test('the endpoint exists', () => { 
    expect(getClanFireteam).toBeDefined();
  })

  getClanFireteamTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getClanFireteam> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getClanFireteam(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getClanFireteam>>))
        }
    })
  ));
})
