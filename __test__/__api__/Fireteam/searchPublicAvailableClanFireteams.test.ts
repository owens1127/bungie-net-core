...
import { searchPublicAvailableClanFireteams } from '../../../src/endpoints/Fireteam';
...
describe('Fireteam.searchPublicAvailableClanFireteams', () => { 
  test('the endpoint exists', () => { 
    expect(searchPublicAvailableClanFireteams).toBeDefined();
  })

  searchPublicAvailableClanFireteamsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof searchPublicAvailableClanFireteams> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await searchPublicAvailableClanFireteams(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof searchPublicAvailableClanFireteams>>))
        }
    })
  ));
})
