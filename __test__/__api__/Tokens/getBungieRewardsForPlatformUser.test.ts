...
import { getBungieRewardsForPlatformUser } from '../../../src/endpoints/Tokens';
...
describe('Tokens.getBungieRewardsForPlatformUser', () => { 
  test('the endpoint exists', () => { 
    expect(getBungieRewardsForPlatformUser).toBeDefined();
  })

  getBungieRewardsForPlatformUserTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getBungieRewardsForPlatformUser> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getBungieRewardsForPlatformUser(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getBungieRewardsForPlatformUser>>))
        }
    })
  ));
})
