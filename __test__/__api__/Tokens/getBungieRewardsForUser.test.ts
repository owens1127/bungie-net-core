...
import { getBungieRewardsForUser } from '../../../src/endpoints/Tokens';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Tokens.getBungieRewardsForUser', () => { 
  test('the endpoint exists', () => { 
    expect(getBungieRewardsForUser).toBeDefined();
  })

  getBungieRewardsForUserTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getBungieRewardsForUser> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getBungieRewardsForUser(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getBungieRewardsForUser>>))
        }
    })
  ));
})
