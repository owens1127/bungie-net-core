...
import { claimPartnerOfferTests } from '../../Tokens'
import { describe, test, it, expect } from '@jest/globals';
import { claimPartnerOffer } from '../../../src/endpoints/Tokens';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Tokens.claimPartnerOffer', () => { 
  test('the endpoint exists', () => { 
    expect(claimPartnerOffer).toBeDefined();
  })

  claimPartnerOfferTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof claimPartnerOffer> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await claimPartnerOffer(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof claimPartnerOffer>>))
        }
    })
  ));
})
