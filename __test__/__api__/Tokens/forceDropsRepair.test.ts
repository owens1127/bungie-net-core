  ...
  forceDropsRepairTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof forceDropsRepair>>))
        }
    })
  ));
})
