...
import { getFriendListTests } from '../../Social'
import { describe, test, it, expect } from '@jest/globals';
import { getFriendList } from '../../../src/endpoints/Social';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Social.getFriendList', () => { 
  test('the endpoint exists', () => { 
    expect(getFriendList).toBeDefined();
  })

  getFriendListTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getFriendList> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getFriendList(sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getFriendList>>))
        }
    })
  ));
})
