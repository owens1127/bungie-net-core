...
import { acceptFriendRequest } from '../../../src/endpoints/Social';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Social.acceptFriendRequest', () => { 
  test('the endpoint exists', () => { 
    expect(acceptFriendRequest).toBeDefined();
  })

  acceptFriendRequestTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof acceptFriendRequest> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await acceptFriendRequest(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof acceptFriendRequest>>))
        }
    })
  ));
})
