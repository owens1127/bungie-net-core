...
import { removeFriendTests } from '../../Social'
import { describe, test, it, expect } from '@jest/globals';
import { removeFriend } from '../../../src/endpoints/Social';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Social.removeFriend', () => { 
  test('the endpoint exists', () => { 
    expect(removeFriend).toBeDefined();
  })

  removeFriendTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof removeFriend> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await removeFriend(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof removeFriend>>))
        }
    })
  ));
})
