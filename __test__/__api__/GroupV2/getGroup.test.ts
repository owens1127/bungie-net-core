...
import { getGroup } from '../../../src/endpoints/GroupV2';
...
describe('GroupV2.getGroup', () => { 
  ...
  getGroupTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getGroup> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getGroup(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getGroup>>))
        }
    })
  ));
})
