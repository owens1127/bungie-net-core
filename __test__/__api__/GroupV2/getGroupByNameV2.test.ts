...
import { getGroupByNameV2Tests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { getGroupByNameV2 } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.getGroupByNameV2', () => { 
  test('the endpoint exists', () => { 
    expect(getGroupByNameV2).toBeDefined();
  })

  getGroupByNameV2Tests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getGroupByNameV2> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getGroupByNameV2(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getGroupByNameV2>>))
        }
    })
  ));
})
