...
import { banMember } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.banMember', () => { 
  test('the endpoint exists', () => { 
    expect(banMember).toBeDefined();
  })

  banMemberTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof banMember> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await banMember(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof banMember>>))
        }
    })
  ));
})
