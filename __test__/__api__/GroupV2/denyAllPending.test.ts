...
import { denyAllPendingTests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { denyAllPending } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.denyAllPending', () => { 
  test('the endpoint exists', () => { 
    expect(denyAllPending).toBeDefined();
  })

  denyAllPendingTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof denyAllPending> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await denyAllPending(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof denyAllPending>>))
        }
    })
  ));
})
