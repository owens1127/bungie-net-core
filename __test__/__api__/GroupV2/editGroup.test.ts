...
import { editGroup } from '../../../src/endpoints/GroupV2';
...
describe('GroupV2.editGroup', () => { 
  ...
  editGroupTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof editGroup> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await editGroup(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof editGroup>>))
        }
    })
  ));
})
