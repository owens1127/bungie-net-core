...
import { denyPendingForList } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.denyPendingForList', () => { 
  test('the endpoint exists', () => { 
    expect(denyPendingForList).toBeDefined();
  })

  denyPendingForListTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof denyPendingForList> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await denyPendingForList(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof denyPendingForList>>))
        }
    })
  ));
})
