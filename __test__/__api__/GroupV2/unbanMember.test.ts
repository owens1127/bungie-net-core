...
import { unbanMemberTests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { unbanMember } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.unbanMember', () => { 
  test('the endpoint exists', () => { 
    expect(unbanMember).toBeDefined();
  })

  unbanMemberTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof unbanMember> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await unbanMember(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof unbanMember>>))
        }
    })
  ));
})
