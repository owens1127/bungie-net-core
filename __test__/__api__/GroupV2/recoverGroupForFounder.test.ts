...
import { recoverGroupForFounder } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.recoverGroupForFounder', () => { 
  test('the endpoint exists', () => { 
    expect(recoverGroupForFounder).toBeDefined();
  })

  recoverGroupForFounderTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof recoverGroupForFounder> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await recoverGroupForFounder(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof recoverGroupForFounder>>))
        }
    })
  ));
})
