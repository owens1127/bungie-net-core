...
import { approvePendingTests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { approvePending } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.approvePending', () => { 
  test('the endpoint exists', () => { 
    expect(approvePending).toBeDefined();
  })

  approvePendingTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof approvePending> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await approvePending(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof approvePending>>))
        }
    })
  ));
})
