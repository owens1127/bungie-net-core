...
import { kickMemberTests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { kickMember } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.kickMember', () => { 
  test('the endpoint exists', () => { 
    expect(kickMember).toBeDefined();
  })

  kickMemberTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof kickMember> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await kickMember(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof kickMember>>))
        }
    })
  ));
})
