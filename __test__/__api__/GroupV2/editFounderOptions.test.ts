...
import { editFounderOptions } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.editFounderOptions', () => { 
  test('the endpoint exists', () => { 
    expect(editFounderOptions).toBeDefined();
  })

  editFounderOptionsTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof editFounderOptions> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await editFounderOptions(data[0], data[1], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof editFounderOptions>>))
        }
    })
  ));
})
