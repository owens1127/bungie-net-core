...
import { groupSearchTests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { groupSearch } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.groupSearch', () => { 
  test('the endpoint exists', () => { 
    expect(groupSearch).toBeDefined();
  })

  groupSearchTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof groupSearch> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await groupSearch(data[0], sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof groupSearch>>))
        }
    })
  ));
})
