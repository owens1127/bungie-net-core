...
import { getAvailableThemesTests } from '../../GroupV2'
import { describe, test, it, expect } from '@jest/globals';
import { getAvailableThemes } from '../../../src/endpoints/GroupV2';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('GroupV2.getAvailableThemes', () => { 
  test('the endpoint exists', () => { 
    expect(getAvailableThemes).toBeDefined();
  })

  getAvailableThemesTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getAvailableThemes> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getAvailableThemes(sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getAvailableThemes>>))
        }
    })
  ));
})
