...
import { getTrendingCategories } from '../../../src/endpoints/Trending';
import { BungieAPIError } from '../../../src/errors/BungieAPIError';

describe('Trending.getTrendingCategories', () => { 
  test('the endpoint exists', () => { 
    expect(getTrendingCategories).toBeDefined();
  })

  getTrendingCategoriesTests.map(({ name, data, promise: { failure, success } }) => (
    describe(name, () => {
        let res: ResponseType<typeof getTrendingCategories> | null = null;
        let err: unknown | null = null;
        
        beforeAll(async () => {
          try {
            res = await getTrendingCategories(sharedTestClient)
          } catch (e) {
            err = e
          }
        });
        ...
        if (failure) {
          ...
          test("it throws the correct error", () => failure!(err as BungieAPIError<ReturnType<typeof getTrendingCategories>>))
        }
    })
  ));
})
