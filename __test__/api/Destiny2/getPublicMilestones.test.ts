...
type ResponseType = UnwrapPromise<ReturnType<typeof client.Destiny2.getPublicMilestones>>;

describe('Destiny2.getPublicMilestones', () => { 
  it('to exist', () => { 
    expect(client.Destiny2.getPublicMilestones).toBeInstanceOf(Function);
  })
  ...
  getPublicMilestonesTests.map(({ name, data, promise: { failure, success } }) => (
    test(name, async () => {
        ...
        try {
            res = await client.Destiny2.getPublicMilestones()
