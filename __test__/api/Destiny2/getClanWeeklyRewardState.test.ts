...
type ResponseType = UnwrapPromise<ReturnType<typeof client.Destiny2.getClanWeeklyRewardState>>;

describe('Destiny2.getClanWeeklyRewardState', () => { 
  it('to exist', () => { 
    expect(client.Destiny2.getClanWeeklyRewardState).toBeInstanceOf(Function);
  })
  ...
  getClanWeeklyRewardStateTests.map(({ name, data, promise: { failure, success } }) => (
    test(name, async () => {
        ...
        try {
            res = await client.Destiny2.getClanWeeklyRewardState(data[0])
