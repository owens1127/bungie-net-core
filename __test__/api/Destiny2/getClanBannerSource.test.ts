...
type ResponseType = UnwrapPromise<ReturnType<typeof client.Destiny2.getClanBannerSource>>;

describe('Destiny2.getClanBannerSource', () => { 
  it('to exist', () => { 
    expect(client.Destiny2.getClanBannerSource).toBeInstanceOf(Function);
  })
  ...
  getClanBannerSourceTests.map(({ name, data, promise: { failure, success } }) => (
    test(name, async () => {
        ...
        try {
            res = await client.Destiny2.getClanBannerSource()
