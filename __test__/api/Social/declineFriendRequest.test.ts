...
type ResponseType = UnwrapPromise<ReturnType<typeof client.Social.declineFriendRequest>>;

describe('Social.declineFriendRequest', () => { 
  it('to exist', () => { 
    expect(client.Social.declineFriendRequest).toBeInstanceOf(Function);
  })
  ...
  declineFriendRequestTests.map(({ name, data, promise: { failure, success } }) => (
    test(name, async () => {
        ...
        try {
            res = await client.Social.declineFriendRequest(data[0])
