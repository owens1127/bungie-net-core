...
export const DictionaryComponentImport =
  './interfaces/DictionaryComponentResponse';
...
export function resolveSchemaType(
  ...
  if (isReferenceObject(schema)) {
    ...
    const mappedComponentName =
      schema.description?.match(/COMPONENT TYPE: (\w+)/)?.[1] ?? null;
    return typeMapping(
      schema,
      doc,
      importFiles!,
...
export function isComponentResponse(
  componentPath: string,
  doc: OpenAPIObject
): boolean {
  const component = getRef(doc, componentPath);
  return Object.values(component?.properties ?? {}).some(
