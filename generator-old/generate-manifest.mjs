...
// @ts-ignore
async function manifestMetaResponse(retry) {
    try {
        // @ts-ignore
        let manifestMeta = await fetch('https://www.bungie.net/Platform/Destiny2/Manifest/').then(res => res.json());
...
export async function generateManifestUtils(components, componentByDef, doc) {
...
${defsToInclude
  ...
  export type AllManifestComponents = {
    ${defsToInclude
        .map(manifestComponent => `'${manifestComponent.typeName}': ManifestComponent<${manifestComponent.typeName}>`)
        .join('\n')}
  }
...
function generateManifestHeader(doc) {
    return `/**
